...
The simulation under the various cutpoints can be visualised using `autoplot()`. The default is to visualise the distributions of NMB across all (500) simulations. In the plot below, the spread (light blue) shows the variability in results due to repeated simulation. The median is represented with the dark blue line in the center of each distribution.

```{r}
autoplot(nmb_simulation)
```
...
autoplot(nmb_simulation, methods = c("all", "none", "youden"))
```
...
```{r}
autoplot(nmb_simulation, what = "cutpoints")
```

```{r}
autoplot(nmb_simulation, what = "inb", inb_ref_col = "all")
```
