...
plot(
  sim_screen_obj, 
  rename_vector = c("Treat All" = "all", "Treat None" = "none", "Youden Index" = "youden", "Value Optimisation" = "value_optimising")
)
```
...
To make tables from the same objects as we used for the plots, we instead use `make_summary_table()`. This can be applied to either type of results (`screen_simulation_inputs()` or `do_nmb_sim()`). Using the `%>%` operator, it is compatible with the `knitr` function `kable()`, which is a simple table generator.
